...
{
    public partial class SearchDocument_New : System.Web.UI.Page
    {
        ...
        protected void gvwDocument_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                ...
                if (e.CommandName.ToLower().Trim() == "download")
                {
                    ...
                    dt = DMS.BusinessLogic.Document.GetDocumentPath(Convert.ToInt32(strDocumentID));
                    Session["DocumentPath"] = dt.Rows[0]["DocumentPath"].ToString();
                    DownloadFile();
                    //Response.Redirect("../UserControl/PdfHandler.ashx?PageID=download",true);
                    //switch (strStatus)
                    //{
                    //    case "1":
                    //        Response.Redirect("../MetaData/ApprovedDocument.aspx?DOCID=" + strDocumentID, false);
        ...
        protected void DownloadFile()
        {
            ...
            // context.Response.AppendHeader("content-disposition", "attachment;filename=\"" +context.Session["DocumentName"].ToString().Substring(0,context.Session["DocumentName"].ToString().IndexOf(".")) + ".tif"+"\"");
            Response.TransmitFile(Session["DocumentPath"].ToString());
            // context.Response.TransmitFile()
            //Server.MapPath("~/Files/MyFile.pdf"));
            Response.Flush();
